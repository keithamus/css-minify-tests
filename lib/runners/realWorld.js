...
export const minifyAllRealWorldTests = async function () {
  ...
  for (const minifierName of minifiers) {
    for (const library of libraries) {
      ...
      library.rerun = library.rerun || {};
      library.idempotent = library.idempotent || {};
      ...
      /* Idempotency */
      try {
        rerun = await minify(minifierName, result);
        rerun = rerun.trim();
      } catch {
        /* do nothing */
      }
      library.rerun[minifierName] = rerun.length;
      library.idempotent[minifierName] = rerun === result;
      saveReminifiedFile(minifierName, library.fileName, (rerun || errorMessage));
    }
  }
