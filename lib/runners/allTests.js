 ...
 * @property {boolean}  idempotent  If the minified output equals itself when being minified
 */
  ...
  for (const minifierName of minifiers) {
    for (const test of allTests) {
      ...
      if (typeof(actual) === 'string') {
        try {
          rerun = await minify(minifierName, actual);
          rerun = rerun.trim();
        } catch {
          rerun = null;
        }
      }
      test.idempotent[minifierName] = rerun === actual;
    }
  }

  function setResultsForIdempotency () {
    const idempotentIndex = allTests.findIndex((test) => {
      return test.groupName === 'idempotency';
    });

    for (const minifierName of minifiers) {
      const failedTests = [];
      for (const test of allTests) {
        if (!test.idempotent[minifierName]) {
          const fullName = test.groupName + '/' + test.testNumber;
          failedTests.push(fullName);
        }
      }

      allTests[idempotentIndex].durations[minifierName] = 0;
      allTests[idempotentIndex].errors[minifierName] = undefined;
      allTests[idempotentIndex].actual[minifierName] = '';
