 ...
 * @param {boolean} reminifed     If true, save to the reminified folder
 */
export const saveMinifiedFile = function (minifierName, fileName, result, reminifed) {
  let folder = 'minified';
  if (reminifed) {
    folder = 'reminified';
  }
  const folderPath = join(__dirname, '..', '..', folder, minifierName);
...
/**
 * Saves the output of each minifed real world file into
 * `/reminified/[MINIFIER_NAME]/[ORIGINAL_FILE_NAME].css`.
 *
 * @param {string} minifierName  The name of the minifier (csso, sass, etc)
 * @param {string} fileName      The original filename ('bttn-v0.2.4.css')
 * @param {string} result        The minified output to save ('a{color:red}')
 */
export const saveReminifiedFile = function (minifierName, fileName, result) {
  saveMinifiedFile(minifierName, fileName, result, true);
};
