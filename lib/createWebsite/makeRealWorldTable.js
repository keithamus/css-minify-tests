/**
 * @file Logic for generating a table with the test results of running minifiers
 *       against real-world CSS libraries, including durations, percent
 *       reduction, and totals.
 */

import {
  CHECKMARK,
  CROSS_MARK,
  ERROR
} from '../constants.js';
import {
  formatMs,
  html
} from '../helpers.js';
import { minifiers } from '../loaders/loadAllMinifiers.js';
import { minifyAllRealWorldTests } from '../runners/realWorld.js';

const makeSizeTable = function (results) {
  const resultsWithoutErrors = results
    .filter((result) => {
      const sizes = Object.values(result.results);
      return !sizes.some((size) => {
        return size === ERROR;
      });
    });

  const makeHeaders = function () {
    return minifiers
      .map((minifier) => {
        return html`
          <th>
            ${minifier}
          </th>
        `;
      })
      .join('\n');
  };

  const makeRows = function () {
    function getClass (lowestSize, size) {
      if (size === ERROR) {
        return 'class="real-error"';
      }
      if (size === lowestSize) {
        return 'class="real-best"';
      }
      return '';
    }
    function getShortestClass (result, currentMinifier, currentDuration) {
      let isShortest = true;
      const size = result.results[currentMinifier];
      if (
        size === ERROR ||
        size === result.size
      ) {
        isShortest = false;
      }
      Object
        .entries(result.duration)
        .forEach(([minifier, time]) => {
          if (
            currentDuration > time &&
            result.results[minifier] !== ERROR &&
            result.results[minifier] !== result.size
          ) {
            isShortest = false;
          }
        });
      if (isShortest) {
        return ' class="real-best"';
      }
      return '';
    }
    function getLowestSize (result) {
      let lowestSize = result.size;
      Object.values(result.results)
        .forEach((size) => {
          if (size && size < lowestSize) {
            lowestSize = size;
          }
        });
      return lowestSize;
    }

    return results
      .map((result) => {
        const lowestSize = getLowestSize(result);

        const sizeTDs = Object
          .entries(result.results)
          .map(([minifier, size]) => {
            const Class = getClass(lowestSize, size);
            const fileName = result.fileName;
            const args = `'${minifier}', '${fileName}'`;
            return html`<td ${Class}>
              <button
                onclick="window.realModal.showMinifiedCSS(${args})"
              >
                ${size.toLocaleString()}
              </button>
            </td>`;
          })
          .join('\n');
        const percentTDs = Object
          .values(result.results)
          .map((size) => {
            const Class = getClass(lowestSize, size);
            if (size === ERROR) {
              return html`
                <td ${Class}>
                  ERROR
                </td>
              `;
            }
            const percent = Math.round((size / result.size) * 10000) / 100;
            return html`
              <td ${Class}>
                ${percent}%
              </td>
            `;
          })
          .join('\n');
        const durationTDs = Object
          .entries(result.duration)
          .map(([minifier, time]) => {
            let Class = getShortestClass(result, minifier, time);
            return html`
              <td ${Class}>
                ${time.toLocaleString()}ms
              </td>
            `;
          })
          .join('\n');
        const idempotentTDs = Object
          .entries(result.rerun)
          .map(([minifier, size]) => {
            const fileName = result.fileName;
            const args = `'${minifier}', '${fileName}'`;
            const idempotent = result.idempotent[minifier];
            let Class = '';
            let emoji = CHECKMARK;
            if (!idempotent) {
              emoji = CROSS_MARK;
              Class = 'class="real-error"';
            }
            return html`<td ${Class}>
              <button
                onclick="window.realModal.showReminifiedCSS(${args})"
              >
                ${emoji}
                ${size.toLocaleString()}
              </button>
            </td>`;
          })
          .join('\n');
        return html`
          <tr class="real-tr">
            <td class="real-break" rowspan="2">
              ${result.name}<br>
              ${result.version}
            </td>
            <td class="real-sep">
              <button
                onclick="window.realModal.showMinifiedCSS('originals', '${result.fileName}')"
              >
                ${result.size.toLocaleString()}
              </button>
            </td>
            ${sizeTDs}
          </tr>
          <tr class="real-tr">
            <td class="real-sep">100%</td>
            ${percentTDs}
          </tr>
          <tr class="real-tr">
            <td class="real-sep" colspan="2">
              Duration
            </td>
            ${durationTDs}
          </tr>
          <tr class="real-tr">
            <td class="real-sep" colspan="2">
              Idempotency
            </td>
            ${idempotentTDs}
          </tr>
        `;
      })
      .join('\n');
  };

  const getTotals = function () {
    const totals = {
      original: 0
    };
    for (const minifier of minifiers) {
      totals[minifier] = 0;
    }
    for (const result of resultsWithoutErrors) {
      totals.original = totals.original + result.size;
      for (const minifier in result.results) {
        const amount = result.results[minifier];
        if (amount === ERROR) {
          // If it failed to minify, use the original
          // unminified size for the total.
          totals[minifier] = totals[minifier] + result.size;
        } else {
          totals[minifier] = totals[minifier] + amount;
        }
      }
    };
    return totals;
  };
  const getLowestSizeTotal = function (totals) {
    let lowestSize = 900_000_000;
    Object.values(totals).forEach((size) => {
      if (size < lowestSize) {
        lowestSize = size;
      }
    });
    return lowestSize;
  };
  const calculateTotals = function () {
    const totals = getTotals();
    const lowestSize = getLowestSizeTotal(totals);
    const totalsTDs = [];
    for (const minifier in totals) {
      const total = totals[minifier];
      let Class = '';
      if (total === lowestSize) {
        Class = 'class="real-best"';
      }
      totalsTDs.push(html`
        <td ${Class}>
          ${total.toLocaleString()}<br>
          (${(Math.round(total / 1024 / 1024 * 100) / 100)}MB)
        </td>`
      );
    }
    return totalsTDs.join('\n');
  };
  const calculatePercents = function () {
    const totals = getTotals();
    const lowestSize = getLowestSizeTotal(totals);
    const { original } = totals;
    const totalsTDs = [];
    for (const minifier in totals) {
      const total = totals[minifier];
      let Class = '';
      if (total === lowestSize) {
        Class = 'class="real-best"';
      }
      const percent = Math.round((total / original) * 10000) / 100;
      totalsTDs.push(html`
        <td ${Class}>
          ${percent}%
        </td>
      `);
    }
    return totalsTDs.join('\n');
  };
  const getTotalDurations = function () {
    function getTotals () {
      const totals = {
      };
      for (const minifier of minifiers) {
        totals[minifier] = 0;
      }
      for (const result of resultsWithoutErrors) {
        for (const minifier in result.results) {
          const duration = result.duration[minifier];
          totals[minifier] = totals[minifier] + duration;
        }
      };
      return totals;
    }
    const getShortestDurationFromTotals = function (totals) {
      let shortestDuration = 900_000;
      Object.values(totals).forEach((duration) => {
        if (duration < shortestDuration) {
          shortestDuration = duration;
        }
      });
      return shortestDuration;
    };

    const totals = getTotals();
    const shortestDuration = getShortestDurationFromTotals(totals);
    const totalsTDs = [];
    for (const minifier in totals) {
      const duration = totals[minifier];
      let Class = '';
      if (shortestDuration === duration) {
        Class = 'class="real-best"';
      }
      const time = formatMs(duration);
      totalsTDs.push(html`
        <td ${Class}>
          ${time}
        </td>
      `);
    }
    return totalsTDs.join('\n');
  };
  const getTotalErrors = function () {
    const errors = {};
    for (const result of results) {
      for (const minifier in result.results) {
        errors[minifier] = errors[minifier] || 0;
        if (result.results[minifier] === ERROR) {
          errors[minifier] = errors[minifier] + 1;
        }
      }
    }
    const errorTDs = [];
    for (const minifier in errors) {
      const amount = errors[minifier];
      let Class = '';
      if (!amount) {
        Class = 'class="real-best"';
      }
      errorTDs.push(html`
        <td ${Class}>
          ${amount}
        </td>
      `);
    }
    return errorTDs.join('\n');
  };
  const getTotalIdempotencyFailures = function () {
    const failures = {};
    for (const result of results) {
      for (const minifier in result.idempotent) {
        failures[minifier] = failures[minifier] || 0;
        if (result.idempotent[minifier]) {
          failures[minifier] = failures[minifier] + 1;
        }
      }
    }
    const failuresTDs = [];
    for (const minifier in failures) {
      const amount = failures[minifier];
      let Class = '';
      if (!amount) {
        Class = 'class="real-best"';
      }
      failuresTDs.push(html`
        <td ${Class}>
          ${amount}
        </td>
      `);
    }
    return failuresTDs.join('\n');
  };

  const allHeaders = makeHeaders();
  const allRows = makeRows();
  const allTotals = calculateTotals();
  const allPercents = calculatePercents();
  const allDurations = getTotalDurations();
  const totalErrors = getTotalErrors();
  const totalIdempotencyFailures = getTotalIdempotencyFailures();

  return html`
    <table>
      <thead>
        <tr>
          <th>Library</th>
          <th>Original Size</th>
          ${allHeaders}
        </tr>
      </thead>
      <tbody>
        ${allRows}
      </tbody>
      <tfoot>
        <tr>
          <th colspan="2"></th>
          ${allHeaders}
        </tr>
        <tr>
          <th colspan="2">Total Errors</th>
          ${totalErrors}
        </tr>
        <tr>
          <td colspan="${minifiers.length + 2}">
            <div class="real-footer-info">
              For a more accurate comparison, the below totals
              exclude real-world files that cause errors.
            </div>
          </td>
        </tr>
        <tr>
          <th>
            Totals
            <span class="real-normal">(${resultsWithoutErrors.length})</span>
          </th>
          ${allTotals}
        </tr>
        <tr>
          <th>Percent of Original Size</th>
          ${allPercents}
        </tr>
        <tr>
          <th colspan="2">Total Durations</th>
          ${allDurations}
        </tr>
        <tr>
          <th colspan="2">Total Idempotency Failures</th>
          ${totalIdempotencyFailures}
        </tr>
      </tfoot>
    </table>
  `;
};

/**
 * Creates the markup for a table showing the results of all
 * minifiers ran against a subset of all imported Real-World
 * CSS files.
 *
 * @return {string} The markup for the table
 */
export const makeRealWorldTable = async function () {
  console.log('Started minifying real world CSS files (may take a minute).');
  const results = await minifyAllRealWorldTests();
  const realWorldMarkup = html`
    <div>
      <style>
      .real-tr [rowspan] { border-right: 1px solid var(--border); }
      /* idempotent row */
      .real-tr:nth-of-type(4n) {
        border-bottom-width: 4px;
        td {
          padding-left: 0;
          padding-right: 0;
        }
        .real-sep {
          padding-left: 1rem;
          padding-right: 1rem;
        }
        button {
          margin: 0;
          white-space: nowrap;
        }
      }
      .real-tr:has(> [rowspan]) + .real-tr td:first-of-type {
        text-align: center;
      }
      .real-break { word-break: break-all; }
      .real-sep + td { border-left-width: 4px; }
      .real-best { background: #2C5229; color: #BFFFB9; }
      .real-error { background: #522929; color: #FFB9B9; }
      .real-hide { display: none; }
      .real-outputs {
        display: flex;
        line-height: 1.3;
      }
      .real-normal { font-weight: normal; }
      .real-diff-added {
        background: #062506;
        border-top: 1px solid #056A05;
        color: #07B707;
      }
      .real-diff-removed {
        background: #670909;
        border-bottom: 1px solid 1px solid #E60000;
      }
      .real-footer-info {
        background: var(--bg);
        padding: 7px 0px;
        font-size: 80%;
        font-style: italic;
        text-align: center;
      }
      #real-css-preview {
        max-width: 60rem;
        width: 100%;
        background: var(--bg);
        border-color: var(--fg-muted);
        border-radius: 9px;
        color: var(--fg);
        header {
          display: flex;
          align-items: baseline;
          justify-content: space-between;
        }
        h3 {
          margin: 0 0 0.75rem;
        }
      }
      #real-moadal-close-button {
        display: flex;
        align-items: center;
        justify-content: center;
        width: 32px;
        height: 32px;
        margin: 0px;
        padding: 0px;
        font-size: 13px;
        font-weight: 600;
        line-height: 32px;
      }
      #real-minified-output,
      #real-comparison-diff {
        width: 100%;
        max-height: calc(100vh - 10rem);
        margin: 0;
        padding: 1rem;
        font-size: 0.7rem;
        white-space: pre-wrap;
        word-break: break-all;
        overflow: auto;
      }
      </style>
      ${makeSizeTable(results)}
    </div>
  `;
  return realWorldMarkup;
};
