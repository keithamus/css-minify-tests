...
/**
 * Generates the <thead> part of a table, including optional row for minifier
 * version numbers if passed in.
 *
 * @param  {string[]} headings  Names to put inside <th> tags
 * @param  {object}   versions  Object where key maps to heading names
 * @return {string}             The structured <thead> markup
 */
...
/**
 * Creates a <tr> with the detailed contents of the results of one test that was
 * ran by the minifiers, including: test README, Source and expected CSS, an
 * embedded <iframe> of the validate.html file (if exists), the actual minified
 * output of each minifier tested (or an error message if the minifier failed),
 * and the duration of time each minifer took to minify the source (or fail to
 * minifiy).
 *
 * @param  {object} test  Contains info about the test and its results
 * @return {string}       The structured <tr> containing test details/results
 */
async function detailRow (test) {
  ...
  if (test.validate) {
    ...
    validate = html`<div>
      <h4>Validate</h4>
      <iframe
        src="data:text/html;base64,${validateBase64}"
  ...
  const playgroundInput = encodeURIComponent(test.source);
  const playgroundExpected = encodeURIComponent(test.expected);
  const playgroundLink = [
    'https://TheJaredWilcurt.com/playground',
    '?i=' + playgroundInput,
    '&e=' + playgroundExpected
  ].join('');
  ...
  return html`<tr>
    <td
      ...
      style="
        --colCount: ${colCount};
        --minifiersLength: ${minifiers.length};
      "
    >
      <details>
        ...
        <div>
          ...
          <div class="details-source-title">
            <h4>Source</h4>
            <a
              class="details-playground-link"
              href="${playgroundLink}"
              target="_blank"
...
/**
 * Creates a <tr> containing the Test number and a summary of each minifier with
 * coloring/iconography and title text to indicate if it passed, failed, or
 * errored.
 *
 * @param  {object} test  Contains info about the test and its results
 * @return {string}       The structured <tr> containing test results overview
 */
...
/**
 * Creates the sub totals row below a group or related tests (anchor, nesting,
 * etc). Includes the total amount of tests that passed for each minifier in the
 * group, the percent passed, and the total duration to run each test by each
 * minifier.
 *
 * @param  {object}   testResults  Keys are minifier names, includes total
 *                                 passing test amount for this group
 * @param  {object[]} group        Array of tests within the same group (anchor, etc)
 * @return {string}                Structured <tr> containing totals/%/durations
 */
...
/**
 * Composes the table for a group of related tests (anchor, nesting, etc).
 *
 * @param  {object}   testResults  Keys are minifier names, includes total
 *                                 passing test amount for this group
 * @param  {object[]} group        Array of tests within the same group (anchor, etc)
 * @return {string}                Structured <table> markup
 */
export const makeGroupTable = async function (testResults, group) {
...
/**
 * Creates the table at the top of the page that summarizes test results.
 *
 * @param  {object[]} allTests     List of all tests and individual test results
 * @param  {object}   testResults  Keys are minifier names, includes total
 *                                 passing test amount for this group
 * @param  {object}   versions     Object where key maps to heading names
 * @param  {object}   groups       All tests grouped by groupName (anchor, etc)
 * @return {string}                Structured <table> markup
 */
export const makeSummaryTable = async function (allTests, testResults, versions, groups) {
  ...
  for (const groupName in testResults.groups) {
    ...
    let row = html`<td><a href="#${groupName}">${groupName}</a></td>`;
