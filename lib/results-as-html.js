...
export async function resultsAsHTML(
  testResults,
  versions,
  history,
  htmlFilePath,
  ...
  let content = html`<section>
    <h2>Summary</h2>
    ${await table(testResults, true, versions)}
  </section>`;
  ...
  for (const test in testResults.tests) {
    content += html`<section id="${test}">
      <h2>${test}</h2>
      ${await table(testResults.tests[test], false)}
    </section>`;
  }
...
async function table(testResults, isSummary = true, versions = null) {
  let rows = "";
  ...
  for (const minifier in testResults.minifiers) {
    ...
    percentCells += html`<td class="${cellClass}">
      ${Math.floor((results.pass / results.checks) * 10000) / 100}%
    </td>`
  }

  if (isSummary) {
    return html`<table>
      ${thead(["test", ...Object.keys(testResults.minifiers)], versions)}
      <tbody>
        ${rows}
      </tbody>
      <tfoot>
...
async function detailRow(test, subResults, minifierNames, colCount) {
  const readme = subResults.readme
    ? html`<p>
        ${marked.parse(escapeHTML(subResults.readme))}
      </p>`
    : "";
  ...
  const sourceMarkup = await codeToHtml(escapeHTML(subResults.source), shikiOptions);
  const expectedMarkup = await codeToHtml(escapeHTML(subResults.expected), shikiOptions);
