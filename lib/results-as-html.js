...
async function detailRow(test, subResults, minifierNames, colCount) {
  ...
  const sourceMarkup = await codeToHtml(escapeHTML(subResults.source), skikiOptions);
  const expectedMarkup = await codeToHtml(escapeHTML(subResults.expected), skikiOptions);
