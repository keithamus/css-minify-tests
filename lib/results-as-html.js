...
import { copyOriginalRealWorldFiles } from './createWebsite/copyOriginalRealWorldFiles.js';
import { createModal } from './createWebsite/createModal.js';
...
/**
 * Creates a list of minifer's names in title case linking to their online
 * playgrounds.
 *
 * @return {string} Markup for list of minifier links
 */
function createMinifierLinks () {
  const links = [];
  ...
  for (const minifierName of minifiers) {
    links.push(html`
      <li>
        <a
          href="${getMinifierUrl(minifierName)}"
          target="_blank"
        >
          ${getMinifierTitle(minifierName)}
        </a>
      </li>
    `);
  }
  ...
  return html`
    <div>
      <strong>Minifiers tested:</strong>
      <ul>
        ${links.join('')}
      </ul>
    </div>
  ...
  content += html`
    <section>
      ...
      <p>
        ...
        open source, CSS files/libraries. The durations track the amount of time
        it took the minifier to minify that real-world file. Idempotency checks
        if the minified output, when minified again, produces the exact same
        result.
      </p>
      ${realWorldTable}
    </section>
